...
public final class FancyTabBarView: UIView {
	...
	private func setConstraintsForTabBarItem(_ item: TabBarItem, activated: Bool, previousItem prevItem: TabBarItem?) {
		...
		NSLayoutConstraint.activate([
			...
			item.heightAnchor.constraint(equalToConstant: item.frame.height),
			item.widthAnchor.constraint(equalToConstant: item.frame.width),
	...
	private func beginTransitionOfHighlighter(to index: Int) {
		UIView.animate(
			...
			item.transform = CGAffineTransform(translationX: 0, y: -Constants.height / 2).scaledBy(x: 2, y: 2)
